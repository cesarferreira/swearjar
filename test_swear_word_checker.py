...
class TestSwearWordChecker(unittest.TestCase):
    ...
    @patch('swearcheck.open', new_callable=mock_open, read_data="badword1\nbadword2\n")
    def test_load_swear_words(self, mock_file):
        # Test loading swear words from file
        result = swearcheck.load_swear_words('fake_path')
        self.assertEqual(result, self.swear_words)

    @patch('swearcheck.open', new_callable=mock_open, read_data=test_data)
    def test_find_swear_words(self, mock_file):
        # Test finding swear words in a file
        with patch('builtins.print') as mock_print:
            swearcheck.find_swear_words('fake_path', self.swear_words)
